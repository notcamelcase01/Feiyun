...
for elm in range(numberOfElements):
    ...
    for xgp in range(len(weightOfGaussPts)):
        for ygp in range(len(weightOfGaussPts)):
            ...
            floc += (fsdt.get_n_matrix(N).T @ np.array([[1, 0, 0, 0, 0]]).T) * weightOfGaussPts[xgp] * weightOfGaussPts[ygp] * np.linalg.det(J)
...
ssi = np.zeros_like(tt)
for i in range(len(tt)):
    ssi[i] = alm.transform(sigma[0][0], sigma[1][0], sigma[2][0], tt[i])[2]
